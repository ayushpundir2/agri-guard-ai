    ...
    <div className="bg-slate-900/90 border border-slate-800 p-5 rounded-xl shadow-xl flex flex-col gap-3 font-mono text-xs">
      <div className="flex items-center justify-between border-b border-slate-800 pb-2.5">
        <h3 className="text-xs font-bold uppercase tracking-wider text-slate-200 flex items-center gap-2">
          ...
          Wholesale Market Exposure Rankings
        </h3>
        <span className="text-[10px] text-slate-500">Sorted by exposure score</span>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-slate-800 text-slate-400 text-[10px] uppercase">
              ...
              <th className="p-2 text-center">Exposure Score</th>
              <th className="p-2 text-center">Risk Level</th>
              <th className="p-2 text-right">Affected Supply Loss</th>
              <th className="p-2 text-right">Connected Farms</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-800/60">
            {markets.map((m) => (
              ...
              >
                <td className="p-2 font-medium text-slate-200">{m.market_name}</td>
                <td className="p-2 text-center font-bold text-amber-400">{m.exposure_score} / 100</td>
                ...
                <td className="p-2 text-right text-red-300 font-semibold">{m.affected_supply_tons} Tons</td>
                <td className="p-2 text-right text-slate-400">{m.affected_parcels_count}</td>
              </tr>
            ))}
          </tbody>
        </table>
